...
from xo_game.common.network_helpers import load_network, get_stochastic_network_move, save_network
...
def train_policy_gradients(game_spec,
    ...
    policy_gradient = tf.log(
        tf.reduce_sum(tf.multiply(actual_move_placeholder, output_layer), reduction_indices=1) + 1e20) * reward_placeholder
    ...
    with tf.Session() as session:
        ...
        def make_training_move(board_state, side):
            ...
            move = get_stochastic_network_move(session, input_layer, output_layer, board_state, side, game_spec=game_spec)
        ...
        for episode_number in range(1, number_of_games):
            ...
            if episode_number % batch_size == 0:
                ...
                if rewards_std != 0:
                    ...
                    num = (1. if mini_batch_rewards[0] > 0 else -1.)
                    normalized_rewards += num
...
def _win_rate(print_results_every, results):
    return sum(results) / (print_results_every * 1.)
